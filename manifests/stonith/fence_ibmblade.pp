...
class pacemaker::stonith::fence_ibmblade (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $port = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_ibmblade for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_ibmblade-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_ibmblade-${real_address} > /dev/null 2>&1",
