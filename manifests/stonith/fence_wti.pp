...
class pacemaker::stonith::fence_wti (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $cmd_prompt = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_wti for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_wti-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_wti-${real_address} > /dev/null 2>&1",
