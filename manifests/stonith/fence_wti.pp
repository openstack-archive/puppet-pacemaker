...
define pacemaker::stonith::fence_wti (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_wti ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_wti-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_wti-${title} > /dev/null 2>&1",
