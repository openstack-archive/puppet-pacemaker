...
define pacemaker::stonith::fence_ilo3 (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_ilo3 ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_ilo3-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_ilo3-${title} > /dev/null 2>&1",
    ...
    exec { "Create stonith::fence_ilo3 ${title}":
      command => "/usr/sbin/pcs stonith create stonith-fence_ilo3-${title} fence_ilo3 pcmk_host_list=\"${pcmk_host_value_chunk}\" ${auth_chunk} ${ipaddr_chunk} ${passwd_chunk} ${passwd_script_chunk} ${lanplus_chunk} ${login_chunk} ${timeout_chunk} ${cipher_chunk} ${method_chunk} ${power_wait_chunk} ${delay_chunk} ${privlvl_chunk} ${verbose_chunk}  op monitor interval=${interval}",
      unless => "/usr/sbin/pcs stonith show stonith-fence_ilo3-${title} > /dev/null 2>&1",
      require => Class["pacemaker::corosync"],
    } ->
    exec { "Add non-local constraint stonith::fence_ilo3 ${title}":
      command => "/usr/sbin/pcs constraint location stonith-fence_ilo3-${title} avoids ${pcmk_host_value_chunk}"
    }
  }
}
