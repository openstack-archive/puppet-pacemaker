...
class pacemaker::stonith::fence_ilo3 (
  $auth = undef,
  $ipaddr = undef,
  $passwd = undef,
  $passwd_script = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_ilo3 for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_ilo3-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_ilo3-${real_address} > /dev/null 2>&1",
    ...
    exec { "Create stonith::fence_ilo3 for ${resource_name}":
      ...
      unless => "/usr/sbin/pcs stonith show stonith-fence_ilo3-${real_address} > /dev/null 2>&1",
      require => Class["pacemaker::corosync"],
    } ->
    exec { "Add non-local constraint stonith::fence_ilo3 for ${resource_name}":
      command => "/usr/sbin/pcs constraint location stonith-fence_ilo3-${real_address} avoids ${pcmk_host_value_chunk}"
    }
  }
}
