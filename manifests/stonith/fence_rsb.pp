...
define pacemaker::stonith::fence_rsb (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_rsb ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_rsb-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_rsb-${title} > /dev/null 2>&1",
