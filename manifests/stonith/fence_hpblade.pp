...
class pacemaker::stonith::fence_hpblade (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $cmd_prompt = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_hpblade for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_hpblade-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_hpblade-${real_address} > /dev/null 2>&1",
    ...
    exec { "Create stonith::fence_hpblade for ${resource_name}":
      ...
      unless => "/usr/sbin/pcs stonith show stonith-fence_hpblade-${real_address} > /dev/null 2>&1",
      require => Class["pacemaker::corosync"],
    } ->
    exec { "Add non-local constraint stonith::fence_hpblade for ${resource_name}":
      command => "/usr/sbin/pcs constraint location stonith-fence_hpblade-${real_address} avoids ${pcmk_host_value_chunk}"
    }
  }
}
