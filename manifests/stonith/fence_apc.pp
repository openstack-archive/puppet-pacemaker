...
class pacemaker::stonith::fence_apc (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $cmd_prompt = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_apc for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_apc-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_apc-${real_address} > /dev/null 2>&1",
