...
class pacemaker::stonith::fence_eaton_snmp (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $port = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_eaton_snmp for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_eaton_snmp-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_eaton_snmp-${real_address} > /dev/null 2>&1",
