...
define pacemaker::stonith::fence_eaton_snmp (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_eaton_snmp ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_eaton_snmp-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_eaton_snmp-${title} > /dev/null 2>&1",
