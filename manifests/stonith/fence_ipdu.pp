...
define pacemaker::stonith::fence_ipdu (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_ipdu ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_ipdu-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_ipdu-${title} > /dev/null 2>&1",
