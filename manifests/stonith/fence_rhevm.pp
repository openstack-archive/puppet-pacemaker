...
define pacemaker::stonith::fence_rhevm (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_rhevm ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_rhevm-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_rhevm-${title} > /dev/null 2>&1",
