...
class pacemaker::stonith::fence_cisco_ucs (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $ssl = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_cisco_ucs for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_cisco_ucs-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_cisco_ucs-${real_address} > /dev/null 2>&1",
    ...
    exec { "Create stonith::fence_cisco_ucs for ${resource_name}":
      command => "/usr/sbin/pcs stonith create stonith-fence_cisco_ucs-${real_address} fence_cisco_ucs pcmk_host_list=\"${pcmk_host_value_chunk}\" ${ipaddr_chunk} ${login_chunk} ${passwd_chunk} ${ssl_chunk} ${notls_chunk} ${port_chunk} ${suborg_chunk} ${ipport_chunk} ${inet4_only_chunk} ${inet6_only_chunk} ${passwd_script_chunk} ${ssl_secure_chunk} ${ssl_insecure_chunk} ${verbose_chunk} ${debug_chunk} ${separator_chunk} ${power_timeout_chunk} ${shell_timeout_chunk} ${login_timeout_chunk} ${power_wait_chunk} ${delay_chunk} ${retry_on_chunk}  op monitor interval=${interval}",
      unless => "/usr/sbin/pcs stonith show stonith-fence_cisco_ucs-${real_address} > /dev/null 2>&1",
      require => Class["pacemaker::corosync"],
    } ->
    exec { "Add non-local constraint stonith::fence_cisco_ucs for ${resource_name}":
      command => "/usr/sbin/pcs constraint location stonith-fence_cisco_ucs-${real_address} avoids ${pcmk_host_value_chunk}"
    }
  }
}
