...
class pacemaker::stonith::fence_virt (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_virt for ${resource_name}":
    ...
    exec { "Create stonith::fence_virt for ${resource_name}":
    ...
    exec { "Add non-local constraint stonith::fence_virt for ${resource_name}":
