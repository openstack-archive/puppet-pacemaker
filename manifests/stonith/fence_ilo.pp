...
class pacemaker::stonith::fence_ilo (
  $ipaddr = undef,
  $login = undef,
  $passwd = undef,
  $ssl = undef,
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_ilo for ${resource_name}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_ilo-${real_address}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_ilo-${real_address} > /dev/null 2>&1",
