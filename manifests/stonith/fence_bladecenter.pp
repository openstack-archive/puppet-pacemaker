...
define pacemaker::stonith::fence_bladecenter (
  ...
  if($ensure == absent) {
    exec { "Delete stonith::fence_bladecenter ${title}":
      command => "/usr/sbin/pcs stonith delete stonith-fence_bladecenter-${title}",
      onlyif => "/usr/sbin/pcs stonith show stonith-fence_bladecenter-${title} > /dev/null 2>&1",
    ...
    exec { "Create stonith::fence_bladecenter ${title}":
      command => "/usr/sbin/pcs stonith create stonith-fence_bladecenter-${title} fence_bladecenter pcmk_host_list=\"${pcmk_host_value_chunk}\" ${ipaddr_chunk} ${login_chunk} ${passwd_chunk} ${cmd_prompt_chunk} ${secure_chunk} ${port_chunk} ${ipport_chunk} ${inet4_only_chunk} ${inet6_only_chunk} ${passwd_script_chunk} ${identity_file_chunk} ${ssh_options_chunk} ${verbose_chunk} ${debug_chunk} ${separator_chunk} ${missing_as_off_chunk} ${power_timeout_chunk} ${shell_timeout_chunk} ${login_timeout_chunk} ${power_wait_chunk} ${delay_chunk} ${retry_on_chunk}  op monitor interval=${interval}",
      unless => "/usr/sbin/pcs stonith show stonith-fence_bladecenter-${title} > /dev/null 2>&1",
      require => Class["pacemaker::corosync"],
    } ->
    exec { "Add non-local constraint stonith::fence_bladecenter ${title}":
      command => "/usr/sbin/pcs constraint location stonith-fence_bladecenter-${title} avoids ${pcmk_host_value_chunk}"
    }
  }
}
